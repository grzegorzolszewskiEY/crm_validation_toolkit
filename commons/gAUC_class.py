"""
Module implementing the gAUC test methods.
"""
import numpy as np
...
class gAUC():
    ...
    def calc_gAUC(self, cont_table: np.array):
        tt = (
            self.P_component(cont_table) - self.Q_component(cont_table)
        ) / self.w_r(cont_table)
        gAUC = (tt+1)/2
        return gAUC
    ...
    def d_ij(self, row: int, col: int, cont_table: np.array):
        return (
            self.A_ij(row, col, cont_table) - self.D_ij(row, col, cont_table)
        )
    ...
    def standard_dev(self, cont_table: np.array):
        ...
        for row in range(cont_table.shape[0]):
            for col in range(cont_table.shape[1]):
                component = cont_table[row][col] * (
                    wr * self.d_ij(row, col, cont_table) -
                    (P-Q)*(F-self.r_i(row, cont_table))
                ) ** 2
